...
require 'includes/Dosen.class.php';

$admin = new Admin($db_host, $db_user, $db_pass, $db_name);
$dosen = new Dosen($db_host, $db_user, $db_pass, $db_name);
$admin->open();
$dosen->open();
...
if (isset($_GET['id_hapus'])) {
    // get mahasiswa yang terkoneksi ke dosen
    $dosen->getMahasiswa($_GET['id_hapus']);
    $dataMahasiswa = $dosen->getResult();

    // jika ada koneksi
    if (count($dataMahasiswa) > 0) {
        header("Location: dosen.php?status=mempunyai koneksi");
    } else {
        $status = $dosen->delete($_GET['id_hapus']);
            ...
            foreach ($data as $row) : ?>
                <tr>
                    ...
                    <td>
                        ...
                        <a href="dosen.php?id_hapus=<?= $row['id']; ?>&table=dosen" class="addButtonArea">
